...
<%@ page import="com.rokai.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="com.rokai.crm.workbench.domain.Tran" %>
...
<%
    ...
    //准备j阶段和可能性之间的对应关系，
	Map<String,String> stageMapTwo = (Map<String,String>)application.getAttribute("stageMap");

	//根据stageMapTwo准备stageMapTwo中的key集合，
	Set<String> keySetTwo = stageMapTwo.keySet();

	//准备: 前面正常阶段和后面丢失阶段分界点下标，
	int point = 0;
	for (int i=0;i<keySetTwo.size();i++){
		...
		//根据stage取得可能性，
		String possibility = stageMapTwo.get(stage);

		//如果可能性为'0'，说明找到了前面正常阶段和后面丢失阶段的分界点。
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#updateRemarkBtn").on("click",function () {
			$.ajax({
				url:"workbench/transaction/editRemark.do",
				type:"post",
				data:{
					"remarkId":$("#remarkId").val(),
					"editRemark":$("#editNoteContent").val()},
				dataType:"json",
				success:function (data) {
	...
	//获取交易历史信息请求，
	function getTransactionHistoryInfo() {
		$.ajax({
			url:"workbench/transaction/getTransactionHistoryInfo.do",
			type:"get",
			data:{"tranId":"${tran.id}"},
			dataType:"json",
			success:function (data) {
	...
	function getRemarkInfo() {
		$.ajax({
			url:"workbench/transaction/getRemarkInfo.do",
			type:"get",
			data:{"tranId":"${tran.id}"},
			dataType:"json",
			success:function (data) {
				...
				$.each(data,function (key,value) {
					...
					html += '<h5><b>: </b><span id="nr'+value.id+'">'+value.noteContent+'</span></h5>';
					html += '<font color="gray">交易</font> <font color="gray">-</font> <b>\'"${tran.customerId}"\'-"${tran.name}"</b> <small style="color: gray;"> '
					...
					html += '<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+value.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: pink;"></span></a>';
					html += '&nbsp;&nbsp;&nbsp;&nbsp;';
					html += '<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\''+value.id+'\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: pink;"></span></a>';
					html += '</div>';
					html += '</div>';
					html += '</div>';
				})

				$("#tranRemarkDiv").html(html)
			}
		})
		...
		$.ajax({
			...
			success:function (data) {
				...
				if (data.flag){
					...
					$("#stage").html(data.tranHistory.stage);
					$("#possibility").html(data.tranHistory.possibility);
                    $("#editBy").html((data.tranHistory.createBy)+"&nbsp;&nbsp;");
                    $("#editTime").html(data.tranHistory.createTime);
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//准备当前阶段可能性，
			String currentPossibility = stageMapTwo.get(currentStage);

			//判断当前阶段，
			//如果当前阶段的可能性为'0'，前7个一定是黑圈，后两个一个红叉、一个黑叉。
			if ("0%".equals(currentPossibility)){
				for (int i=0;i<dicValueList.size();i++){
					...
					String possibility = stageMapTwo.get(value);

					//如果遍历出来的阶段可能性为'0'，	说明是后面两个，一个是红叉、一个是黑叉。
					if ("0%".equals(possibility)){

						//如果是当前阶段，
						if (value.equals(currentStage)){

							//红叉，
							%>
							<span id="<%=i%>" onclick="changeStage('<%=value%>','<%=i%>')"
			...
			//如果当前阶段的可能性不为'0'，前7个一定是绿圈、绿色标记、黑圈，后俩个一定是黑叉。
			}else {
				...
				for (int i=0;i<dicValueList.size();i++){
					...
					//如果遍历出来的阶段可能性不为'0'，说明是前7个阶段，绿圈、绿色标记、黑圈。
					}else {
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 10px;">
			...
			<div style="width: 300px;position: relative; left: 100px; top: -20px;"><b>${tran.customerId}-${tran.name}</b></div>
